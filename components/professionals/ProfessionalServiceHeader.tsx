...
function getHeroSummary(
  ...
  if (yearsExperience && place) {
    return `${yearsExperience}+ anos em projetos urbanos e execucao tecnica em ${place}.`;
  }

  if (yearsExperience) {
    return `${yearsExperience}+ anos em projetos urbanos, integracao tecnica e operacoes em campo.`;
  }

  if (place) {
    return `Base operacional em ${place}, com oferta pronta para servicos urbanos, manutencao e implantacao tecnica.`;
  }

  return "Configure seu catalogo profissional, organize servicos ativos e prepare o portfolio para futuras conexoes com tags e marketplace.";
}
    ...
    <header className="flex flex-col gap-6 border-b border-border/70 pb-6 lg:flex-row lg:items-start lg:justify-between">
      <div className="max-w-3xl">
        ...
        <p className="mt-3 text-sm uppercase tracking-[0.2em] text-brand-steel-deep">
          Configure your urban professional offerings
        </p>
      ...
      <div className="flex items-center gap-4 rounded-[1.75rem] border border-border/70 bg-surface-frost px-4 py-4 shadow-[0_12px_30px_rgba(4,22,39,0.06)]">
        ...
        <div>
          ...
          <p className="mt-1 text-[11px] uppercase tracking-[0.2em] text-text-subtle">
            {professional.tier_label ?? "Top End Civil"}
          </p>
        </div>
      </div>
    </header>
